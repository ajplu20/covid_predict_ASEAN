...
compute_incidence <- function(df, model, cap_fcast = c(0, 1e5), min_inc = -Inf, sum_by = "week") {
  ...
  #' 
  if (grepl("epi", model)) {
    df$forecast <- compute_incidence.epi(df$forecast, toupper(df$state[1]))
  }
  df <- df %>%
...
cc <- read.csv("ASEAN_files/asean-census-population.csv")
...
pred_score <- function(
  ...
  if (type == "calibration") {
    ...
    scoring_fct <- function(x, y, tau) { 
      error = y - x
      loss  <- pmax(tau*error, (tau-1)*error)
      result <- mean(loss)
      #print(result)
      return(result)
    }
