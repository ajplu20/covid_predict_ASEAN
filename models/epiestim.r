...
train_and_predict.epiestim <- function(tau, ...) {
  ...
  ##mean = generation_time$mean_mean
  ##sd = generation_times$sd_mean
  
  cG <- make_config(
    method = "uncertain_si",
    t_start = seq(2, length(args$data$cases) - tau + 1),
    t_end = seq(2 + tau - 1, length(args$data$cases)),
    mean_si = generation_time$mean_mean,
    std_si = generation_time$sd_mean,
    std_mean_si = generation_time$mean_sd,
    std_std_si = generation_time$sd_sd,
    min_mean_si = generation_time$mean_mean - 3 * generation_time$mean_sd,
    max_mean_si = generation_time$mean_mean + 3 * generation_time$mean_sd,
    min_std_si = generation_time$sd_mean - 3 * generation_time$sd_sd,
    max_std_si = generation_time$sd_mean + 3 * generation_time$sd_sd,
    n1 = cori.n1,
    n2 = cori.n2,
